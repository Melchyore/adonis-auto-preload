...
test.group('Auto preload - Multiple rows', (group) => {
  group.setup(async () => {
    app = await setupApp()
  ...
  test('using mixin should auto-preload relationships when using relation name', async ({
    ...
    await user1.related('posts').createMany([
      {
        title: 'Test',
    ...
    await user2.related('posts').createMany([
      {
        title: 'Test',
        content: 'Test content',
      },
    ...
    expect((await User.all()).at(0)?.$preloaded.posts).toHaveLength(1)
    expect((await User.all()).at(1)?.$preloaded.posts).toHaveLength(2)
  })
  ...
  test('using mixin should auto-preload nested relationships when using relation name', async ({
    ...
    expect((await User.all()).at(0)?.$preloaded.posts[0].$preloaded.comments).toHaveLength(1)
    expect((await User.all()).at(0)?.$preloaded.posts[1].$preloaded.comments).toHaveLength(2)
  })
  ...
  test('using mixin should auto-preload multiple relationships when using relation names', async ({
    ...
    expect(posts.at(0)?.$preloaded.comments).toHaveLength(2)
    expect(posts.at(1)?.$preloaded).toHaveProperty('user')
    expect(posts.at(1)?.$preloaded).toHaveProperty('comments')
    expect(posts.at(1)?.$preloaded.comments).toHaveLength(1)
  })
  ...
  test('without method should exclude specified relationships from being auto-preloaded', async ({
    ...
    const posts = await Post.without(['comments']).all()
  ...
  test('without method should throw an exception when relationship is not a string', async ({
    ...
    // @ts-ignore
    expect(async () => await Post.without([5]).all()).rejects.toThrowError()
  })
  ...
  test('withOnly method should auto-preload only specified relationships', async ({ expect }) => {
    ...
    const posts = await Post.withOnly(['user']).all()
  ...
  test('withOnly method should throw an exception when relationship is not a string', async ({
    ...
    // @ts-ignore
    expect(async () => await Post.withOnly([5]).all()).rejects.toThrowError()
  })
  ...
  test('withoutAny method should not auto-preload any relationship', async ({ expect }) => {
    ...
    const posts = await Post.withoutAny().all()
