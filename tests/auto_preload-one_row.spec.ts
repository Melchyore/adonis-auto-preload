...
test.group('Auto preload - One row', (group) => {
  group.setup(async () => {
    app = await setupApp()
  ...
  test('using mixin should auto-preload relationships when using relation name', async ({
    ...
    expect((await User.find(1))?.$preloaded.posts).toHaveLength(2)
  })
  ...
  test('using mixin should auto-preload nested relationships when using relation name', async ({
    ...
    expect((await User.find(1))?.$preloaded.posts[0].$preloaded.comments).toHaveLength(1)
    expect((await User.find(1))?.$preloaded.posts[1].$preloaded.comments).toHaveLength(2)
  })
  ...
  test('using mixin should auto-preload multiple relationships when using relation names', async ({
    ...
    expect((await Post.find(1))?.$preloaded).toHaveProperty('user')
    expect((await Post.find(1))?.$preloaded).toHaveProperty('comments')
    expect((await Post.find(1))?.$preloaded.comments).toHaveLength(1)
    expect((await Post.find(2))?.$preloaded).toHaveProperty('user')
    expect((await Post.find(2))?.$preloaded).toHaveProperty('comments')
    expect((await Post.find(2))?.$preloaded.comments).toHaveLength(2)
  })
  ...
  test('without method should exclude specified relationships from being auto-preloaded', async ({
    ...
    const post = (await Post.without(['comments']).find(1))!

    expect(post.$preloaded).toHaveProperty('user')
    expect(post.$preloaded).not.toHaveProperty('comments')
  })
  ...
  test('without method should throw an exception when relationship is not a string', async ({
    ...
    // @ts-ignore
    expect(async () => await Post.without([5]).find(1)).rejects.toThrowError()
  })
  ...
  test('withOnly method should auto-preload only specified relationships', async ({ expect }) => {
    ...
    const post = (await Post.withOnly(['user']).find(1))!
  ...
  test('withOnly method should throw an exception when relationship is not a string', async ({
    ...
    // @ts-ignore
    expect(async () => await Post.withOnly([5]).find(1)).rejects.toThrowError()
  })
  ...
  test('withoutAny method should not auto-preload any relationship', async ({ expect }) => {
    ...
    const post = (await Post.withoutAny().find(1))!

    expect(post.$preloaded).not.toHaveProperty('user')
    expect(post.$preloaded).not.toHaveProperty('comments')
  })
